...
/* GRAPESHOT channel tag */
var gs_channels="DEFAULT"; 
var gsurl=window.location!=window.top.location?document.referrer:window.location;
document.write('<scr' + 'ipt type="text/javascript" src="http://haymarket.grapeshot.co.uk/main/channels.cgi?url=' + encodeURIComponent(gsurl) + '"></scr' + 'ipt>');
...
var HaymarketAnalyticsConfig = {
	...
	, currencyCode: "GBP"
    /* Google Analytics */
	, googleAnalyticsAccount: 'UA-9767361-1'
    /* Nedstat Sitestat */
    /*
    ,nedstatSitestatAccount: 'haymarket/pistonheads'
    ,nedstatSitestatTrackableLinks: true
    */
    /* DART */
	, dartSitename: "pistonheads"
	, dartPromoteHierarchy1ToSubsite: true
	, dartIncludeProductModelInZone: true
    , dartMaxItemsInZone: 1
    /*,getCustomAdvertZone : function(thisInstance) { return thisInstance.getGenerateNavigationZone(2, false).toLowerCase(); } */
    /* Revenue Sciences */
	...
	, googleAnalytics:{
        Variables: [
        { name: "dealer", scope: 3, values: ["product_dealerid", "product_dealer"] },
        { name: "nav", scope: 3, values: ["navigation_.*"] },
        { name: "prod", scope: 3, values: ["product_model", "product_edition", "product_format", "product_style", "product_size"] },
        { name: "loc", scope: 3, values: ["location_country", "location_region", "location_subregion", "location_postcode"] },
        { name: "contnt", scope: 3, values: ["content_.*"] }
        ]
        , TrackView: ["navigation_hierarchy_.*", "product_.*"]
        , Event: { category: ["navigation_hierarchy_.*"], action: ["relTokens_2"], label: ["product_.*"] }
        , LinkAnalyticAdSenseAccount: true
	}
};
/**
* HaymarketAnalytics
...
function HaymarketAnalytics(params) {
    ...
    this.searchMetrics = { query: '', filters: [], count: '', result: '', minprice: '', maxprice: '' };
    this.teamMetrics = { nationality: '', club: '', division: '', position: '', sponsor: '', skill: '', country: '', region: '', competition: '' };


    if (typeof (this.siteConfig.googleAnalytics) == 'object' && this.siteConfig.googleAnalytics.LinkAnalyticAdSenseAccount == true) {
        window.google_analytics_uacct = this.siteConfig.googleAnalyticsAccount;
        window.google_analytics_domain_name = this.siteConfig.googleAnalyticsDomain;
    }

    this.allowedInAd = new Array("product_dealer", "product_brand", "product_model", "product_edition", "product_format", "product_style", "product_size"
		  , "content_id", "content_type", "content_group", "content_title", "content_subtitle"
		  , "location_country", "location_region", "location_subregion","location_postcode", "location_postcodeletters", "team_.*", "affiliate_.*", "search_minprice", "search_maxprice", "product_price");
...
HaymarketAnalytics.renderGoogleAnalyticsCallback = function(instance) {
    ...
    if (typeof (_gat) != 'object') {
        ...
        if (HaymarketAnalytics.isNotEmptyString(pageView)) {
            ...
            instance.logToConsole("Google Analytics pageview: url");
            pageTracker._trackPageview();
        }
        if (typeof (cfg) == 'object' && cfg.TrackPageLoad == true) {
...
HaymarketAnalytics.prototype.observeLinks = function(instance, observer, args) {
    ...
    if ((typeof (jQuery) != "undefined")) {
        var shadow = function() {
            ...
            var s = new String(jQuery(element).attr("rel"));
        ...
        /**/
        var relSelector = "a[rel*='trackable_'],input[rel*='trackable_'],div[rel*='trackable_']";
        //if (typeof (jQuery(relSelector).live) != "undefined") {
        jQuery(document).ready(function() {
            if (jQuery.isFunction(jQuery.fn.live)) {
                jQuery(relSelector).live("click", shadow);
                instance.logToConsole("GoogleAnalytics:trackable event registered live selector");
            } else {

                jQuery(relSelector).each(function() {
                    jQuery(this).click(shadow);
                    instance.logToConsole("GoogleAnalytics:trackable event registered click on " + jQuery(this).attr("rel"));
                });
                instance.logToConsole("GoogleAnalytics:trackable event registered clicks");
            }
        });
        /**/
    }
...
HaymarketAnalytics.googleAnalyticsObserver = function(instance, element, tokens, external, args) {
    ...
    instance.logToConsole("GoogleAnalytics: event tracking registerd " + tokens[2] + " " + instance.navigationMetrics.section.toLowerCase());
...
HaymarketAnalytics.nedstatOvserver = function(instance, element, tokens, external, args) {
    if (tokens.length > 0) {
        ...
        if (HaymarketAnalytics.isNotEmptyString(originalHref)) {
            originalHref = new String(jQuery(element).attr("rel")); // where observed element is input and not a tag (use the content of the rel tag to identify the click)
        }
...
HaymarketAnalytics.prototype.createRenderAdvert = function (dimensions, position, tile) {
    ...
    if (typeof gs_channels != 'undefined') {
        if (gs_channels != 'DEFAULT' && gs_channels != 'NONE') {
            gsCat = 'gs_cat=' + gs_channels + ';';
            this.logToConsole("Grapeshot Category: " + gsCat);
        }
    }
    ...
    var src = adserver + siteandZone + ';' + customValues + 'pos=' + thePosition + ';sz=' + dimensions + ';tile=' + theTile + ';' + gsCat + 'ord=' + this.randomNumber;
    ...
    return { adurl: src, adserver: adserver, siteandZone: siteandZone, custom: gsCat + customValues, sz: dimensions, pos: thePosition, tile: theTile, ord: this.randomNumber };
};
...
HaymarketAnalytics.prototype.getAdvertCustomValues = function() {
    ...
    var allowedInAd = this.allowedInAd;
...
HaymarketAnalytics.prototype.getAdvertZone = function() {
    ...
    if (typeof (cfg.getCustomAdvertZone) == "function") {
        ...
        if (typeof (affiliateCode) == "string" && affiliateCode.length > 0) {
            ...
            var zoneBase = [];
            for (var j = i; j < Math.min(maxSegemnts, this.navigationMetrics.hierarchy.length); j = j + 1) {
...
HaymarketAnalytics.prototype.safelyMoveAdvert = function(fromElement, toElement) {
    toElementObject = jQuery(document.getElementById(toElement));
    fromElementObject = jQuery(document.getElementById(fromElement));
    var removals = [];
    if (toElementObject.size() && fromElementObject.size()) {
        /* remove scripts injected by this class, so ads don't duplicate in IE */
        fromElementObject.find('script').each(function(i) { var _src = jQuery(this).attr("src"); var text = jQuery(this).text(); jQuery(this).remove(); removals.push({ src: _src, text: text }); });
    ...
    this.debug.adMoved(fromElement, toElement, removals);
};
...
HaymarketAnalyticsDebug.prototype.adMoved = function(fromElement, toElement, removals) {
    this.movedAds.push({ fromElement: fromElement, toElement: toElement, removals: removals });
};
...
HaymarketAnalyticsDebug.prototype.logToConsole = function(text) {
    ...
    if (this.enabled && window.console && window.console.log) { console.log(text); }
